...
const ResetPassword = () => {
  ...
  return (
    <div className="relative w-full min-h-screen bg-white">
      <div className="py-12 md:py-20 container-custom max-w-xl mx-auto px-4 sm:px-6 relative">
      <div className="bg-white rounded-[2rem] shadow-md border-2 border-neutral-300 p-8 sm:p-12 relative overflow-hidden">
        <FlakPink className="absolute pointer-events-none z-0" style={{ width: 175, top: 10, right: -18, opacity: 0.33, transform: 'rotate(15deg)' }} />
        <FlakCream className="absolute pointer-events-none z-0" style={{ width: 165, bottom: 10, left: -15, opacity: 0.30, transform: 'rotate(-25deg)' }} />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {/* --- VIZUÁLNA VALIDÁCIA --- */}
            {passwordFocus && (
                <div className="mt-3 p-4 bg-neutral-50 border border-neutral-200 rounded-xl text-sm overflow-hidden">
          ...
          {/* Messages */}
          {message && (
              <div className="rounded-xl p-4 bg-emerald-50 text-emerald-800 border border-emerald-200 font-medium text-sm flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 flex-shrink-0 text-emerald-600 mt-0.5" />
                <span>{message}</span>
              </div>
            )}
          {error && (
              <div className="rounded-xl p-4 bg-red-50 text-red-800 border border-red-200 font-medium text-sm flex items-start gap-3">
