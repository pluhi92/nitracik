        ...
        >
            <div className="px-4 py-6 space-y-2 flex flex-col items-center text-center relative" style={{ isolation: 'isolate' }}>
              <FlakPink
                ...
                style={{ width: 180, bottom: 10, right: -20, opacity: 0.35, zIndex: -1, transform: 'rotate(20deg)' }}
              />
              <FlakCream
                className="absolute pointer-events-none"
                style={{ width: 160, top: 10, left: -20, opacity: 0.35, zIndex: -1, transform: 'rotate(-15deg)' }}
              />
              <FlakPink
                className="absolute pointer-events-none"
                style={{ width: 130, top: '35%', right: 10, opacity: 0.30, zIndex: -1, transform: 'rotate(50deg)' }}
              />
              <FlakCream
                className="absolute pointer-events-none"
                style={{ width: 140, bottom: '25%', left: 10, opacity: 0.30, zIndex: -1, transform: 'rotate(-40deg)' }}
              />
              {navLinks.map(({ path, label }) => {
                const isActive = location.pathname.startsWith(path);
                return (
              ...
              <div className="border-t border-neutral-300 my-4 pt-4 px-2 w-full flex flex-col items-center">
                ...
                {user.isLoggedIn ? (
                  <div className="space-y-3">
                    <Link
                      to="/profile"
                      className="flex items-center justify-center w-full px-5 py-3.5 rounded-full font-bold text-foreground border-2 border-neutral-300 bg-white hover:bg-neutral-50 transition-colors"
                    >
                    ...
                    <button
                      onClick={handleLogout}
                      className="flex items-center justify-center w-full px-5 py-3.5 rounded-full font-bold text-red-600 bg-red-50 hover:bg-red-100 transition-colors"
                    >
                  ...
                  <div className="space-y-3">
                    <Link
                      to="/login"
                      onClick={() => setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 80)}
                      className="flex items-center justify-center w-full px-5 py-3.5 rounded-full font-bold text-white bg-primary shadow-sm hover:bg-primary-600 transition-colors"
                    >
                      {t?.navbar?.login || 'Prihlásiť sa'}
                    </Link>
                    <Link
                      to="/register"
                      className="flex items-center justify-center w-full px-5 py-3.5 rounded-full font-bold text-primary bg-primary/10 hover:bg-primary/20 transition-colors"
                    >
